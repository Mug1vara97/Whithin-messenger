...
/// <summary>
/// Idempotent data fix after <c>FixSavedMessagesChatTypeId</c> migration.
/// Must run after <see cref="DatabaseFacadeExtensions.MigrateAsync"/>.
/// </summary>
public static class SavedMessagesChatTypeDataFix
{
    public static async Task ApplyAsync(WithinDbContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (!savedTypeExists)
        {
            ...
            {
                Id = ChatTypeIds.Saved,
                TypeName = ChatTypeNames.Saved,
            });
            await context.SaveChangesAsync(cancellationToken);
        }
