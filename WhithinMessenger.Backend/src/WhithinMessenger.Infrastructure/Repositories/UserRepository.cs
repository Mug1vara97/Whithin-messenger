...
namespace WhithinMessenger.Infrastructure.Repositories
{
    public class UserRepository : IUserRepository, IUserRepositoryExtensions
    {
        ...
        public async Task<List<UserSearchInfo>> SearchAllUsersAsync(Guid currentUserId, string searchTerm, CancellationToken cancellationToken = default)
        {
            var normalizedName = searchTerm.Trim().ToLower();
            ...
            // Поиск ВСЕХ пользователей (без ограничений по друзьям/чатам)
            var users = await _context.Users
                .Where(u => u.Id != currentUserId)
                .Where(u =>
                    u.UserName.ToLower().Contains(normalizedName) ||
                ...
                .ThenByDescending(u => u.HasExistingChat)
                .ThenBy(u => u.Username)
                .Take(50)
                .ToListAsync(cancellationToken);
