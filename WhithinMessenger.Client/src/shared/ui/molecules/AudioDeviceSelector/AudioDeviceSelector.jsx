...
export const AudioDeviceSelector = ({ className = '' }) => {
  ...
  return (
    <div className={`${styles.container} ${className}`}>
      ...
      {/* Кнопка для тестирования аудио устройства */}
      <button
        className={styles.testButton}
        onClick={async () => {
          const current = await getCurrentAudioDevice();
          alert(
            `Текущее устройство для голосов:\n${current.label}\n\n` +
            `Для предотвращения эха голоса должны быть на НАУШНИКАХ,\n` +
            `а демонстрация экрана захватит звук с ДИНАМИКОВ (Default Device).`
          );
        }}
      >
        🧪 Проверить настройку
